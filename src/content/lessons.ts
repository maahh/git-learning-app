...
export const lessons: Lesson[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { kind: "heading", text: "ターミナルでファイルを見る・書き換える" },
      {
        kind: "paragraph",
        text: "このアプリのターミナルには、メモ帳のような画面はありません。ファイルの中身を見るには `cat`、書き換えるには `printf` や `echo` を使って、ファイルを丸ごと上書きします。",
      },
      {
        kind: "code",
        caption: "中身を見る → 正しい内容で上書きする",
        lines: [
          "$ cat README.md            # いまの中身（マーカー入り）を表示する",
          "$ printf \"# My Project\\n## 説明\\nまとめた説明\\n\" > README.md",
          "# 末尾の > は「ファイルを丸ごと書き換える」という意味",
        ],
      },
      {
        kind: "callout",
        tone: "tip",
        title: "上書きで一気にマーカーを消す",
        body: "`>` はファイルを丸ごと置き換えます。`printf \"...\" > README.md` を実行すると、衝突マーカーごと消えて、指定した内容だけが残ります。vi や nano を知っている人はそれで編集してもかまいません。",
      },
      ...
      },
      {
        ...
        steps: [
          ...
          {
            label: "2. `cat README.md` で中身とマーカーを読む",
            why: "`cat` でファイルの中身を表示します。`<<<<<<< HEAD` 側が main、`>>>>>>> feature` 側が相手の変更です。どちらを残すか、または両方をまとめるかを人間が判断します。",
          },
          {
            label: "3. `printf \"...\" > README.md` で正しい内容に上書きする",
            why: "例: `printf \"# My Project\\n## 説明\\nまとめた説明\\n\" > README.md`。末尾の `>` でファイルを丸ごと置き換え、`<<<<<<<` `=======` `>>>>>>>` の3種類のマーカーをまとめて消します。これでファイルの中身としては解決済みです。",
          },
          {
